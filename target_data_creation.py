...
for path in range(len(get_dir)):
    
    p_name = get_dir[path].split('\\')[-1]
    i_name = p_name.split('_')[-1]
    ...
    if '1' in i_name:
        ...
        x1 = bicubic(img, 2, 1.2) 
        sav1 = './target/'+p_name
        cv2.imwrite(sav1, x1*255)
        
    elif '2' in i_name:
        img = cv2.resize(img, (h//3, w//3), interpolation = cv2.INTER_AREA)
        x2 = bicubic(img, 3, 1.5)
        x2 = cv2.resize(x2, (h, w), interpolation = cv2.INTER_AREA)
        sav2 = './target/'+p_name
        cv2.imwrite(sav2, x2*255)
        
    elif '3' in i_name:
        img = cv2.resize(img, (h//2, w//2), interpolation = cv2.INTER_AREA)
        x3 = bicubic(img, 2, 1.8) 
        sav3 = './target/'+p_name
        cv2.imwrite(sav3, x3*255)